...
namespace Microsoft.Json.Serialization
{
    using System;
    ...
    public sealed class JsonSerializer
    {
        ...
        {
            return Deserialize(expression, _type);
        }
        ...
        {
            return Deserialize(Expression.Parse(input));
        }
        ...
        private Expression SerializeObject(object o, Stack<object> refs)
        {
            ...
            if (iDict != null)
            {
                ...
                var keys = (IEnumerable)iDict.GetProperty("Keys").GetValue(o, null);
            ...
            else
            {
                var res = (from field in type.GetFields(BindingFlags.Public | BindingFlags.Instance)
                           select new { Key = field.Name, Value = Serialize(field.GetValue(o), refs)})
                          .Concat(
                           from property in type.GetProperties(BindingFlags.Public | BindingFlags.Instance)
                           where property.CanRead
                           select new { Key = property.Name, Value = Serialize(property.GetValue(o, null), refs) });
        ...
        private static object DeserializeArray(ArrayExpression arrayExpression, Type type)
        {
            ...
            if (type.IsArray) // Do this first. Array implements IList<T> but instantiating an Array as such causes issues.
            {
                ...
                var array = Array.CreateInstance(elementType, arrayExpression.Elements.Count);

                for (int i = 0; i < arrayExpression.Elements.Count; i++)
                {
                    array.SetValue(Deserialize(arrayExpression.Elements[i], elementType), i);
                }

                return array;
            }

            var iList = type.GetInterface("System.Collections.Generic.IList`1", false);
            if (iList != null)
            {
                Type elementType = iList.GetGenericArguments()[0];
                var list = Activator.CreateInstance(type);

                var iColl = type.GetInterface("System.Collections.Generic.ICollection`1", false); // static guarantee IList<T> : ICollection<T>
                var add = iColl.GetMethod("Add");

                for (int i = 0; i < arrayExpression.Elements.Count; i++)
                {
                    add.Invoke(list, new object[] { Deserialize(arrayExpression.Elements[i], elementType) });
                }
            ...
            else if (typeof(IList).IsAssignableFrom(type))
            {
                ...
                for (int i = 0; i < arrayExpression.Elements.Count; i++)
                {
                    list.Add(Deserialize(arrayExpression.Elements[i], typeof(object)));
                }
        ...
        {
            string value = (string)constantExpression.Value;
